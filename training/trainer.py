...
from training.diagnostics import (
    raw_value_to_wdl_class, compute_buffer_diagnostics,
...
class Trainer:
    ...
    def train_network(self, n_new_positions=0):
        ...
        # === DIAGNOSTIC: Buffer diagnostics (value distribution, player bias, 3-in-a-row) ===
        all_values = np.array([s[2] for s in samples])
        buf_diag = compute_buffer_diagnostics(samples, all_values)
        val_mean = buf_diag['val_mean']
        val_std = buf_diag['val_std']
        frac_pos = buf_diag['frac_pos']
        frac_neg = buf_diag['frac_neg']
        frac_draw = buf_diag['frac_draw']
        val_hist = buf_diag['val_hist']
        n_x_buf = buf_diag['n_x_buf']
        n_o_buf = buf_diag['n_o_buf']
        mean_tgt_x = buf_diag['mean_tgt_x']
        mean_tgt_o = buf_diag['mean_tgt_o']
        frac_pos_x = buf_diag['frac_pos_x']
        frac_pos_o = buf_diag['frac_pos_o']
        ...
        # (#2) Pre-training value loss + per-player predictions
        pre_train_vloss, _pbias_data = compute_pre_training_diagnostics(
            self.net, samples, self.device
        )
        ...
        # (V) Value head health diagnostics (extracted to diagnostics.py)
        vh_diag = compute_value_head_diagnostics(self, samples, grad_stats_summary)
        # Backbone gradient decomposition: value vs policy (updates vh_diag in place)
        compute_backbone_gradient_decomposition(self, samples, vh_diag)

        # SVD rank tracking & policy head internals (updates vh_diag in place)
        ...
        _post_x_pred, _post_o_pred, _post_x_acc, _post_o_acc = \
            compute_post_training_player_diagnostics(self.net, _pbias_data, self.device)
